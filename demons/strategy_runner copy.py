...
Цели:
    ...
    - Для каждого нового бара:
        ...
        * через strategy_catalog по strategy_universe.strategy_id (code) найти
          py_module и py_class, импортировать модуль, создать экземпляр класса.
...
Ключевые допущения:
    - Все timestamp свечей в БД — UTC.
    - Агрегированные таблицы имеют поля: symbol_id, timestamp, open, high, low, close, volume, is_gap, gap_dir.
    - strategy_universe хранит строковый timeframe ('5m', '15m', '1h' и т.п.), symbol (тикер)
      и strategy_id, который является code из strategy_catalog.
    - strategy_catalog(code, py_module, py_class) определяет, какой модуль/класс Python использовать:
        * py_module: строка вида 'strategies.sma_trend1'.
        * py_class: имя класса (например, 'SMATrend1Strategy'), имеющего метод on_bar(context).
    - Python-стратегии живут в пакетe strategies, путь к модулю полностью задаётся py_module.

Архитектура:
    - Один процесс с циклом:
        1. Для каждого таймфрейма:
            - берёт last_bar_timestamp из bar_state.
            - читает новые записи из соответствующей candles_xx (timestamp > last_bar_timestamp).
            - группирует по (symbol_id, timestamp) и обрабатывает каждый бар.
        2. По каждому бару:
            - резолвит symbol_id -> symbol (ticker).
            - для (symbol, timeframe) ищет активные стратегии с join на strategy_catalog.
            - по каждой стратегии строит Context, вызывает on_bar, пишет сигналы.
        3. Обновляет bar_state и heartbeat.
        4. Спит небольшой интервал и повторяет.
    - Исключения в стратегиях пишутся в live_errors (source='strategy') и не останавливают цикл.
"""
...
def log_error(conn, message, severity="error", source="strategy_runner",
...
def load_bar_history(conn, tf_table: str, symbol_id: int, ts: datetime, limit: int) -> List[BarInfo]:
    """
...
def insert_signal(conn, strategy_universe_id: int, symbol: str, timeframe: str,
                  bar: BarInfo, signal: Dict[str, Any], signal_source: str = "strategy"):
    with conn.cursor() as cur:
        cur.execute(
            """
...
def get_strategy_instance(su_row) -> Optional[Any]:
    ...
    Использует strategy_catalog.py_module и py_class.
    ...
    if not py_module or not py_class:
        logger.error(f"Для strategy_universe_id={su_id} не заданы py_module/py_class")
    ...
    try:
        ...
        logger.error(f"Не удалось создать экземпляр {py_class} из {py_module}: {e}")
        _strategy_instances[su_id] = None
        return None
...
def process_bar_for_timeframe(conn, timeframe: str, tf_table: str, last_ts: Optional[datetime]) -> Optional[datetime]:
    """
    ...
    for r in rows:
        ...
        if not ticker:
            logger.warning(f"Не найден ticker для symbol_id={symbol_id}, пропускаем бар.")
            continue
        ...
        for s_row in strategies:
            ...
            try:
                ...
                logger.exception(f"Ошибка в стратегии {strategy_id_code} (strategy_universe_id={su_id})")
                log_error(
                    conn,
            ...
            if not isinstance(signal, dict) or "type" not in signal:
                log_error(
                    ...
                    symbol=ticker,
                    timeframe=timeframe,
                    details={"strategy_id": strategy_id_code, "signal": str(signal)},
                )
                continue
