class ChartPanelManager {
    ...
    setupTimeScaleSync(panelData) {
        // Не пытаемся синхронизировать, если график еще не готов
        setTimeout(() => {
            if (!panelData.chart || !panelData.chart.timeScale) return;
            ...
            panelData.chart.timeScale().subscribeVisibleTimeRangeChange((range) => {
                ...
                this.panels.forEach(p => {
                    if (p.id !== panelData.id && p.chart && p.chart.timeScale) {
                        try {
